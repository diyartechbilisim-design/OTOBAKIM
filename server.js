...
async function initDB() {
  ...
  try {
    await client.query(`
      ...
      CREATE TABLE IF NOT EXISTS musteriler (id SERIAL PRIMARY KEY, ad VARCHAR(255) NOT NULL, tel VARCHAR(50) DEFAULT '', adres TEXT DEFAULT '', plaka VARCHAR(20) DEFAULT '', arac VARCHAR(255) DEFAULT '', km INTEGER DEFAULT 0, toplam_borc DECIMAL(10,2) DEFAULT 0, odenen DECIMAL(10,2) DEFAULT 0, created_at TIMESTAMP DEFAULT NOW());
...
const fm = m => ({ id: m.id, ad: m.ad, tel: m.tel||'', adres: m.adres||'', plaka: m.plaka||'', arac: m.arac||'', km: parseInt(m.km||0), toplamBorc: parseFloat(m.toplam_borc||0), odenen: parseFloat(m.odenen||0) });
...
app.post('/api/musteriler', async (req, res) => { try { const { ad, tel, adres, plaka, arac, km } = req.body; const r = await pool.query('INSERT INTO musteriler (ad,tel,adres,plaka,arac,km) VALUES ($1,$2,$3,$4,$5,$6) RETURNING *', [ad, tel||'', adres||'', plaka||'', arac||'', km||0]); res.json(fm(r.rows[0])); } catch(e) { console.error('POST musteriler:', e.message); res.status(500).json({ error: e.message }); } });
app.put('/api/musteriler/:id', async (req, res) => { try { const { ad, tel, adres, plaka, arac, km, toplamBorc, odenen } = req.body; await pool.query('UPDATE musteriler SET ad=$1,tel=$2,adres=$3,plaka=$4,arac=$5,km=$6,toplam_borc=$7,odenen=$8 WHERE id=$9', [ad, tel||'', adres||'', plaka||'', arac||'', km||0, toplamBorc||0, odenen||0, req.params.id]); res.json({ ok: true }); } catch(e) { console.error('PUT musteriler:', e.message); res.status(500).json({ error: e.message }); } });
