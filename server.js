...
app.get('/api/sorgula', async (req, res) => {
  try {
    const plaka = String(req.query.plaka||'').toUpperCase().replace(/\s+/g,'');
    const sase = String(req.query.sase||'').toUpperCase().trim();
    if(!plaka || sase.length < 6) return res.status(400).json({ error: 'Plaka ve şasi numarasının son 6 hanesi gereklidir.' });
    const r = await pool.query("SELECT * FROM musteriler WHERE UPPER(REPLACE(plaka,' ',''))=$1 AND UPPER(RIGHT(sase,6))=$2", [plaka, sase.slice(-6)]);
    if(!r.rows.length) return res.status(404).json({ error: 'Bu plaka ve şasi numarasıyla eşleşen araç bulunamadı.' });
    const m = r.rows[0];
